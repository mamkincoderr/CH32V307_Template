...
### 8.1 Скопировать SVD-файл (один раз)

SVD-файл нужен для просмотра регистров периферии в отладчике.
Скопировать его можно прямо из MRS2 — выполнить в PowerShell:

```powershell
Copy-Item "C:\MounRiver\MounRiver_Studio2\resources\app\resources\win32\components\WCH\SDK\default\RISC-V\CH32V307\NoneOS\CH32V307xx.svd" ".\doc\CH32V307.svd"
```

(SVD не входит в репозиторий из-за большого размера — 1.4 МБ)

### 8.2 Запустить отладку

1. Подключить WCH-Link к плате (см. раздел 7)
2. Нажать **F5** — проект автоматически пересоберётся и запустится отладка
3. Отладчик остановится на функции `main()`

Доступны: точки останова, просмотр переменных и регистров периферии (если скопирован SVD).

### 8.3 Остановить отладку

- **Shift+F5** — нормальное завершение
- Кнопка **$(debug-stop)  Стоп** в статусбаре — принудительное завершение (если завис)
